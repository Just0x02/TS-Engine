...
export class Eventful<EVENT_SCHEME extends string = StandardEvents>
{
	...
	public static UseCustomHandler<T_EVENT_SCHEME extends string = StandardEvents, T extends Eventful<T_EVENT_SCHEME> = Eventful<T_EVENT_SCHEME>>(eventId: T_EVENT_SCHEME)
	{
		return function (target: T, propertyKey: string, descriptor: PropertyDescriptor) {
			const base: Function = descriptor.value!;

			descriptor.value = function(cb: EventCallback) {
				let newListenerId: number = target.on.apply(this, [eventId as T_EVENT_SCHEME, cb]);
